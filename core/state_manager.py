...
FIX 2026-03-17:
  - FutureWarning dtype resuelto en calculate_adx dentro de DataProcessor
    (plus_dm y minus_dm inicializados como float64, no int64)
"""
...
class StateManager:
    ...
    def _generate_key(self, symbol: str, data_type: str, **kwargs) -> str:
        params_str = f"{symbol}_{data_type}_" + "_".join(
    ...
    def invalidate_cache(self, symbol: Optional[str] = None):
        if symbol is None:
            ...
            keys_to_delete = [
                k for k in st.session_state.cache_store.keys()
                if symbol in k
            ]
...
class DataProcessor:
    ...
    def prepare_full_analysis(data: pd.DataFrame, analyzer) -> pd.DataFrame:
        ...
        FIX FutureWarning: plus_dm y minus_dm se inicializan como float64
        en lugar de int64. pandas >= 2.0 no permite asignar floats a Series
        de dtype int64 sin casteo explícito — genera FutureWarning y en
        versiones futuras lanzará un error real.

        La causa era:
            pd.Series(0, index=df.index)   ← dtype int64
            plus_dm[mask] = up[mask]        ← floats → int64: FutureWarning

        El fix:
            pd.Series(0.0, index=df.index) ← dtype float64
            plus_dm[mask] = up[mask]        ← floats → float64: OK
        """
        df = data.copy()
        ...
        # FIX: inicializar como float64 (0.0 en lugar de 0)
        # Evita FutureWarning "dtype incompatible with int64"
    ...
    def get_latest_signals(df: pd.DataFrame) -> Dict:
        ...
        return {
            ...
            'rvol':            latest['RVOL'],
            'bb_position':     (
                'UPPER'  if latest['Close'] >= latest['BB_Upper'] * 0.99 else
                'LOWER'  if latest['Close'] <= latest['BB_Lower'] * 1.01 else
                'MIDDLE'
            ),
