...
CONFIG = {
    ...
    "agent_interval_hours": 4,         # El agente revisa oportunidades cada 4 horas
...
class QuantScheduler:
    ...
    def run_agent(self):
        """
        Corre el agente proactivo: escanea el universo,
        consulta Groq y actualiza la watchlist automáticamente.
        """
            ...
            if summary:
                ...
                if added or removed:
                    ...
                    if added:
                        ...
                        for ticker in added:
                            reason = summary["added_reasons"].get(ticker, "")
                            msg += f"  • {ticker}: {reason}\n"

                    if removed:
                        msg += f"\n🗑️ *Eliminados ({len(removed)}):*\n"
                        for ticker in removed:
                            reason = summary["removed_reasons"].get(ticker, "")
                            msg += f"  • {ticker}: {reason}\n"
                ...
                # Sincronizar watchlist actualizada
                self.db.sync_watchlist_from_json()
    ...
    def setup_schedule(self):
        ...
        schedule.every().day.at("09:25").do(self.run_agent)  # Pre-market ET
    ...
    def run(self):
        ...
        # Primer ciclo del agente al arrancar
